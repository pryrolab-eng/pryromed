...
function ToolFallbackTrigger({
  ...
  const isRunning = false; // All tools execute server-side and are returned as completed
  const isCancelled =
    ...
    >
      <div
        className={cn(
          ...
          "group-data-[state=closed]/collapsible-content:duration-[var(--animation-duration)] group-data-[state=open]/collapsible-content:duration-[var(--animation-duration)]",
        )}
      >
        {children}
      </div>
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  // Detect A2UI question tool results
  const isA2UIQuestion =
    toolName === "ask_user" &&
    result &&
  ...
  if (isA2UIQuestion) {
    const a2uiData = (
      result as { data: A2UIQuestionData }
    ).data;
    return (
      <A2UIQuestion
        data={a2uiData}
        onSubmit={(_questionId, answers) => {
          const answerText = Object.entries(answers)
            .map(([k, v]) => `${k}: ${v}`)
            .join("\n");
          // Send answer as a new user message to trigger AI continuation
