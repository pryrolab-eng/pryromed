...
/**
 * Logo — shows the icon mark only (the grid/hash symbol).
 * The platform name is rendered separately via AuthBrandingName or
 * the LogoWithName component so it can be dynamic.
 */
export const Logo = ({ className, uniColor }: { className?: string; uniColor?: boolean }) => {
    return (
        <span className={cn('inline-flex items-center gap-2', className)}>
            ...
            <span
                className="font-bold text-foreground tracking-tight text-base"
...
export const LogoIcon = ({ className, uniColor }: { className?: string; uniColor?: boolean }) => {
    ...
    const fill = uniColor ? 'currentColor' : `url(#${gradientId})`
