...
/**
 * Logo — brand mark + name.
 * Uses the same hash mark as the PWA / home-screen icons.
 */
export const Logo = ({
  className,
  uniColor,
  ...
  return (
    <span className={cn("inline-flex items-center gap-2", className)}>
      {uniColor ? <LogoIcon uniColor /> : <AppIcon size={20} />}
...
/** Raster app icon (matches installable PWA asset). Prefer for auth / marketing. */
...
/** Vector hash mark — same geometry as `public/icons/*` / PWA assets. */
export const LogoIcon = ({
  ...
  const fill = uniColor ? "currentColor" : PRYROX_BRAND_BLUE;
